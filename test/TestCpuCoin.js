...
const RUN_ALL_TESTS = false;     // This should be always checked in set to true!
...
const contractFullPath = 'FANVesting.sol';
...
beforeEach(async () => {
    ...
    if (deployment !== undefined) {
        ...
        FANT = deployment.contract;
        if (log) console.log(colors.green('==> FAN deployed to ' + FANT.options.address));
    }
});
...
describe('FANT', () => {
    ...
    // ================================================================================
    // === Test basic attributes of the FANT token
    // ================================================================================
    it('0. verify successful compilation/deploymement of FANT', () => {
        checkAreNotEqual(FANT, null, "FANT is null. Did compile fail? Did deploy fail? Was there a problem with the constructor? Try setting RUN_ALL_TESTS = false.");
    });

    if (runThisTest())
        it('1. verified deployment of the FANT contract by confirming it has a contract address', () => {
            if (!FANT) return;
            checkAreNotEqual(FANT.options.address, null);
        });

    if (runThisTest())
        it('2. verified FANT has symbol CPU', async () => {
            if (!FANT) return;
            result.set(await FANT.methods.symbol().call());
            result.checkIsEqual('CPU');
        });

    if (runThisTest())
        it('3. verified FANT has 18 decimals', async () => {
            if (!FANT) return;
            result.set(await FANT.methods.decimals().call());
            result.checkIsEqual(WEI_DECIMALS);
        });

    if (runThisTest())
        it('4. verified FANT owner account and total supply initially have 5 billion tokens', async () => {
            if (!FANT) return;
            result.set(await FANT.methods.balanceOf(owner).call());
            result.checkIsEqual(tokensToWei(TOTAL_SUPPLY));
            result.set(await FANT.methods.totalSupply().call());
            result.checkIsEqual(tokensToWei(TOTAL_SUPPLY));
        });

    if (runThisTest())
        it('5. verified FANT non-owner account initially has zero tokens', async () => {
            if (!FANT) return;
            result.set(await FANT.methods.balanceOf(account1).call());
    ...
    if (runThisTest())
        it('6. verified only owner account effectively owns the contract', async () => {
            ...
            await subtest('owner of contract', async () => {
                result.set(await FANT.methods.owner().call());
                result.checkIsEqual(owner);
            }).catch(catcher);

            await subtest('6a. account0 is owner, others are not.', async () => {
                result.set(await FANT.methods.isOwner().call());
                result.checkIsTrue();
                result.set(await FANT.methods.isOwner().call({from: account1}));
                result.checkIsFalse();
                result.set(await FANT.methods.isOwner().call({from: account2}));
                result.checkIsFalse();
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('7. verified owner account can change ownership to another account', async () => {
            ...
            await subtest('transfer of contract ownership to account 1', async () => {
                result.set(await FANT.methods.registerAccount().send({from: account1}));
                result.checkTransactionOk();
                result.set(await FANT.methods.transferOwnership(account1).send({from: owner, gas: XFEROWNERGAS}));
                result.checkTransactionOk();
                result.set(await FANT.methods.owner().call());
    ...
    if (runThisTest())
        it('8. verified non-owners cannot change ownership', async () => {
            ...
            await subtest('8a. transfer ownership from non-owner account1', async () => {
                result.set(await FANT.methods.registerAccount().send({from: account2}));
                result.checkTransactionOk();
                result.set(await expectFail(FANT.methods.transferOwnership(account2).send({
                    from: account1,
                    gas: XFEROWNERGAS
                })).catch(catcher));
                result.checkDidFail();
            ...
            await subtest('8b. transfer ownership from non-owner account2', async () => {
                result.set(await expectFail(FANT.methods.transferOwnership(owner).send({
                    from: account2,
                    gas: XFEROWNERGAS
                })).catch(catcher));
                result.checkDidFail();
    ...
    if (runThisTest())
        it('9. verified ownership can\'t be renounced', async () => {
            if (!FANT) return;
            result.set(await expectFail(FANT.methods.renounceOwnership().send({from: owner})).catch(catcher));
            result.checkDidFail();
            result.set(await expectFail(FANT.methods.renounceOwnership().send({from: account1})).catch(catcher));
            result.checkDidFail();
        });
    ...
    if (runThisTest())
        it('10. verified ERC20 transfer()', async () => {
            ...
            await subtest('10a. transfer owner->account1, then verify balances', async () => {
                result.set(await FANT.methods.transfer(account1, tokensToWei(10000)).send({from: owner}));
                result.checkTransactionOk();
                // .
                result.set(await FANT.methods.balanceOf(owner).call());
            ...
            await subtest('10b. make transfer account1->account2, then verify balances', async () => {
                result.set(await FANT.methods.transfer(account2, tokensToWei(1000)).send({from: account1}));
                result.checkTransactionOk();
                // Verify new balances.
                result.set(await FANT.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(9000));
                result.set(await FANT.methods.balanceOf(account2).call());
                result.checkIsEqual(tokensToWei(1000));
            }).catch(catcher);

            await subtest('10c. trying to transfer more tokens than the balance will fail', async () => {
                result.set(await expectFail(FANT.methods.transfer(account2, tokensToWei(999999)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            ...
            await subtest('10d. smaller transfer succeeds after first failed transfer', async () => {
                result.set(await FANT.methods.transfer(account2, tokensToWei(9000)).send({from: account1}));
                result.checkTransactionOk();
                // Verify new balances.
                result.set(await FANT.methods.balanceOf(account1).call());
                result.checkIsEqual(0);
    ...
    async function approveThenTransferFunds(from, to, amount) {
        ...
        result.set(await FANT.methods.approve(to, amount).send({from: from}));
        result.checkTransactionOk('approveThenTransferFunds(): approve failed');
        // Transfer approved funds 'from' -> 'to'.
        result.set(await FANT.methods.transferFrom(from, to, amount).send({from: to}));
    ...
    if (runThisTest())
        it('11. verified ERC20 transfer/approve method', async () => {
            ...
            await subtest('11a. transfer owner->account1, then verify balances', async () => {
                ...
                result.checkIsEqual(tokensToWei(10000));
                // Verify allowances
                result.set(await FANT.methods.allowance(owner, account1).call());
                result.checkIsEqual(0);
                result.set(await FANT.methods.allowance(account1, owner).call());
                result.checkIsEqual(0);
            }).catch(catcher);
            ...
            await subtest('11b. make transfer account1->account2, then verify balances', async () => {
                ...
                result.checkIsEqual(tokensToWei(1000));
                // Verify allowances
                result.set(await FANT.methods.allowance(account1, account2).call());
                result.checkIsEqual(0);
                result.set(await FANT.methods.allowance(account2, account1).call());
                result.checkIsEqual(0);
            }).catch(catcher);
            ...
            await subtest('11c. trying to transfer more tokens than the balance will fail', async () => {
                ...
                await FANT.methods.approve(account2, 0).send({from: account1});
                // Verify nothing changed.
                result.set(await FANT.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(9000));
                result.set(await FANT.methods.balanceOf(account2).call());
            ...
            await subtest('11d. smaller transfer succeeds after prior failed transfer', async () => {
                ...
                result.checkIsEqual(tokensToWei(10000));
                // Verify allowances
                result.set(await FANT.methods.allowance(account1, account1).call());
                result.checkIsEqual(0);
                result.set(await FANT.methods.allowance(account2, account1).call());
                result.checkIsEqual(0);
            }).catch(catcher);
    ...
    if (runThisTest())
        it('12. verified transferFrom() fails without prior approval', async () => {
            ...
            await subtest('12a. account1 can\'t transfer from owner to account2', async () => {
                result.set(await expectFail(FANT.methods.transferFrom(owner, account2, tokensToWei(5 * ONE_MILLION)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12b. owner can\'t transfer from account1 to account2', async () => {
                result.set(await expectFail(FANT.methods.transferFrom(account1, account2, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12c. owner can\'t transfer from owner to account1', async () => {
                result.set(await expectFail(FANT.methods.transferFrom(owner, account1, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12d. owner can\'t transfer from owner to owner', async () => {
                result.set(await expectFail(FANT.methods.transferFrom(owner, owner, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('13. burn tokens', async () => {
            ...
            await subtest('13b. non-owner can burn own tokens.', async () => {
                ...
                result.set(await FANT.methods.transfer(account1, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
                result.checkTransactionOk();
                result.set(await FANT.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(100 * ONE_MILLION));
                // Try to burn 50M from account 1
                result.set(await FANT.methods.burn(tokensToWei(40 * ONE_MILLION)).send({from: account1}));
                result.checkTransactionOk();
            ...
            await subtest('13c. owner can burn own tokens', async () => {
                ...
                result.set(await FANT.methods.burn(tokensToWei(50 * ONE_MILLION)).send({from: owner}));
                result.checkTransactionOk();
                // Confirm tokens are gone
                result.set(await FANT.methods.balanceOf(owner).call());
    ...
    if (runThisTest())
        it('20. test today()', async () => {
            ...
            await subtest('20a. theck today() computation', async () => {
                result.set(await FANT.methods.today().call());
    ...
    async function testGrantVestingTokens(owner, account, grantParams, vestingSchedule) {
        result.set(await FANT.methods.grantVestingTokens(
    ...
    async function testGrantVestingTokens_balances(owner, account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable]) {
        ...
        const ownerBalanceBefore = await FANT.methods.balanceOf(owner).call();
        const accountBalanceBefore = await FANT.methods.balanceOf(account).call();
        ...
        const ownerBalance = await FANT.methods.balanceOf(owner).call();
        ...
        let accountBalance = await FANT.methods.balanceOf(account).call();
        ...
        accountBalance = await FANT.methods.vestingAsOf(Math.floor(startDay)).call({from: account, gas: VESTASOFGAS});
    ...
    async function testVestingScheduleDayByDay(account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable = false]) {
        ...
        for (let elapsedDays = -dayIncrement; elapsedDays <= (duration + dayIncrement); elapsedDays += dayIncrement) {
            ...
            result.set(await FANT.methods.vestingAsOf(Math.floor(onDay)).call({from: account, gas: VESTASOFGAS}));
            checkResult('vestingAsOf() ');

            // Test using the grant administrator access method
            result.set(await FANT.methods.vestingForAccountAsOf(account, Math.floor(onDay)).call({from: account}));
    ...
    async function testAccountHasNoGrant(account, wasRevoked, onDay) {
        result.set(await FANT.methods.vestingAsOf(onDay).call({from: account, gas: VESTASOFGAS}));
    ...
    if (runThisTest())
        it('21. test grantVestingTokens() success cases', async () => {
            ...
            result.set(await FANT.methods.registerAccount().send({from: account9}));
            result.checkTransactionOk();
            result.set(await FANT.methods.safeTransfer(account9, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();
            result.set(await FANT.methods.transferOwnership(account9).send({from: owner, gas: XFEROWNERGAS}));
            result.checkTransactionOk();
    ...
    if (runThisTest())
        it('23. test grantVestingTokens() failure cases (account limits, ownership transfer)', async () => {
            ...
            await FANT.methods.registerAccount().send({from: newOwner});
            await FANT.methods.safeTransfer(newOwner, tokensToWei(100 * ONE_MILLION)).send({from: owner});
            await FANT.methods.transferOwnership(newOwner).send({from: owner, gas: XFEROWNERGAS});
    ...
    if (runThisTest())
        it('24. test usage of granted tokens (contract use of today\'s date)', async () => {
            ...
            await subtest('24a. verified granted tokens outside of vesting can be transferred before vesting begins', async () => {
                ...
                result.set(await expectFail(FANT.methods.transfer(account2, tokensToWei(1)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
            ...
            await subtest('24a. verified no tokens can be transferred with an unvested grant', async () => {
                ...
                result.set(await FANT.methods.transfer(account4, tokensToWei(1)).send({from: account3}));
                result.checkTransactionOk('Transfer of 1 token from grant was supposed to work');

                // Make sure a too-large transfer fails.
                result.set(await expectFail(FANT.methods.transfer(account4, tokensToWei(2)).send({from: account3})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
        });
    ...
    async function testRevokeVestingSchedule(owner, account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable], revokeOnDay) {
        ...
        result.set(await FANT.methods.vestingAsOf(revokeOnDay).call({from: account, gas: VESTASOFGAS}));
        const vestedAsOfDay = result.value()[0];
        const notVestedAsOfDay = result.value()[1];

        const ownerBalanceAfterGrant = await FANT.methods.balanceOf(owner).call();
        const accountBalanceAfterGrant = await FANT.methods.balanceOf(account).call();
        ...
        result.set(await FANT.methods.revokeGrant(account, revokeOnDay).send({from: owner, gas: REVOKEONGAS}));
        result.checkTransactionOk('revokeGrant() failed');

        // Verify token balances (tokens should NOT have moved)
        const ownerBalanceAfterRevocation = await FANT.methods.balanceOf(owner).call();
        const accountBalanceRevocation = await FANT.methods.balanceOf(account).call();
        ...
        result.set(await FANT.methods.allowance(owner, account).call());
        result.checkIsEqual(0, 'There should be no leftover allowance in the correct direction');
        result.set(await FANT.methods.allowance(account, owner).call());
    ...
    if (runThisTest())
        it('25. test date ranges and grant duration()', async () => {
            ...
            await subtest('25a. First, verify this schedule is allowed on a normal date', async () => {
                ...
                result.set(await FANT.methods.revokeGrant(account1, TODAY_DAYS).send({from: owner, gas: REVOKEONGAS}));
                result.checkTransactionOk('revokeGrant() failed');
            }).catch(catcher);
    ...
    if (runThisTest())
        it('27. Test onlyGrantorOrSelf() modifier of vestingForAccountAsOf()', async () => {
            ...
            result.set(await FANT.methods.vestingAsOf(Math.floor(TODAY_DAYS)).call({from: owner, gas: VESTASOFGAS}));
            checkAreEqual(Object.size(result.value()), resultSize, 'Expected result to be the result tuple');
            result.set(await FANT.methods.vestingAsOf(Math.floor(TODAY_DAYS)).call({from: account1, gas: VESTASOFGAS}));
            checkAreEqual(Object.size(result.value()), resultSize, 'Expected result to be the result tuple');

            // It should work that owner can call vestingForAccountAsOf() for account1 (because owner is grantor)
            result.set(await FANT.methods.vestingForAccountAsOf(account1, Math.floor(TODAY_DAYS)).call({from: owner}));
            checkAreEqual(Object.size(result.value()), resultSize, 'Expected result to be the result tuple');
            // It should also work that account1 can call vestingForAccountAsOf() for self
            result.set(await FANT.methods.vestingForAccountAsOf(account1, Math.floor(TODAY_DAYS)).call({from: account1}));
            checkAreEqual(Object.size(result.value()), resultSize, 'Expected result to be the result tuple');

            // However, account2 should be prevented from looking at account1.
            result.set(await expectFail(FANT.methods.vestingForAccountAsOf(account1, Math.floor(TODAY_DAYS)).call({from: account2})).catch(catcher));
            result.checkDidFail();
        });
    ...
    async function testRoleBehavior(testNumber, roleName, isRoleMethod, addRoleMethod, addRoleParams, removeRoleMethod) {
        result.set(await FANT.methods.isOwner().call({from: owner}));
        result.checkIsTrue('Expected original owner to be owner');
        result.set(await FANT.methods[isRoleMethod](owner).call({from: owner}));
        ...
        await subtest(testNumber + 'a. transfer contract ownership to account 1', async () => {
            ...
            result.set(await FANT.methods.owner().call({from: owner}));
            result.checkIsEqual(account1);
        }).catch(catcher);
        ...
        await subtest(testNumber + 'c. Check that new owner is now both owner and has role ' + roleName, async () => {
            ...
            result.set(await FANT.methods[isRoleMethod](account1).call({from: account1}));
        ...
        await subtest(testNumber + 'd. test remove role ' + roleName + ' from owner account1, leaving no other account having role (specifically allowed!)', async () => {
            result.set(await FANT.methods[removeRoleMethod](account1).send({from: account1}));
        ...
        await subtest(testNumber + 'e. test addRoleMethod() success cases', async () => {
            result.set(await FANT.methods[addRoleMethod](account5, ...addRoleParams).send({from: account1}));
            result.checkTransactionOk('account1 as new owner should have been able to give account5 role ' + roleName);
            result.set(await FANT.methods[isRoleMethod](account5).call({from: account5}));
            result.checkIsTrue("account5 should now have role " + roleName);
            result.set(await FANT.methods[isRoleMethod](account5).call({from: account1}));
            result.checkIsTrue("account5 should now have role " + roleName + " (even if other account performs the inspection)");
            result.set(await FANT.methods[removeRoleMethod](account5).send({from: account1}));
        ...
        await subtest(testNumber + 'f. test addRoleMethod() failure cases', async () => {
            result.set(await FANT.methods[isRoleMethod](account3).call({from: owner}));
            result.checkIsFalse("account3 should start off not having role " + roleName);
            result.set(await expectFail(FANT.methods[addRoleMethod](account3, ...addRoleParams).send({from: owner})).catch(catcher));
            result.checkDidFail();
            result.set(await FANT.methods[isRoleMethod](account3).call({from: owner}));
            ...
            result.set(await FANT.methods[isRoleMethod](account3).call({from: account6}));
            result.checkIsFalse("account3 should start off not having role " + roleName);
            result.set(await expectFail(FANT.methods[addRoleMethod](account3, ...addRoleParams).send({from: account6})).catch(catcher));
            result.checkDidFail();
            result.set(await FANT.methods[isRoleMethod](account3).call({from: account6}));
        ...
        await subtest(testNumber + 'g. non-owner (without the role) trying to add/remove role' + roleName, async () => {
            result.set(await expectFail(FANT.methods[addRoleMethod](account4, ...addRoleParams).send({from: account3})).catch(catcher));
            result.checkDidFail();
            result.set(await expectFail(FANT.methods[removeRoleMethod](account4).send({from: account3})).catch(catcher));
        ...
        await subtest(testNumber + 'h. non-owner should not be able to add/remove role' + roleName, async () => {
            ...
            result.set(await expectFail(FANT.methods[removeRoleMethod](account4).send({from: account3})).catch(catcher));
            result.checkDidFail('account3 should NOT have been able to remove role ' + roleName + ' from other account');
            result.set(await expectFail(FANT.methods[removeRoleMethod](account1).send({from: account3})).catch(catcher));
        ...
        await subtest(testNumber + 'i. non-owner (having the role) should not be able to add/remove role' + roleName, async () => {
            ...
            result.set(await expectFail(FANT.methods[addRoleMethod](account6, ...addRoleParams).send({from: account5})).catch(catcher));
            result.checkDidFail('Account with role ' + roleName + ' should NOT have been able to add same role.');
            result.set(await expectFail(FANT.methods[removeRoleMethod](account6).send({from: account5})).catch(catcher));
    ...
    async function testPausableMethod(method, methodParams, from) {
        ...
        // Pause the contract
        result.set(await FANT.methods.pause().send({from: owner}));
        result.checkTransactionOk();
        ...
        // Put things back
        result.set(await FANT.methods.unpause().send({from: owner}));
        result.checkTransactionOk();
    }
    ...
    if (runThisTest())
        it('32. test that pausable ERC20 functionality is blocked while paused', async () => {
            ...
            result.set(await FANT.methods.transfer(account1, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();
            result.set(await FANT.methods.transfer(account2, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();

            await subtest('32a. try a valid call to transfer(), paused and not paused', async () => {
                await testPausableMethod(
                    FANT.methods.transfer,
            ...
            await subtest('32d. try a valid call to increaseAllowance(), paused and not paused', async () => {
                await testPausableMethod(
                    FANT.methods.increaseAllowance,
            ...
            await subtest('32e. try a valid call to decreaseAllowance(), paused and not paused', async () => {
                await testPausableMethod(
                    FANT.methods.decreaseAllowance,
                    [account5, tokensToWei(ONE_MILLION)],
                    account2).catch(catcher);
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('33. additional pausable tests', async () => {
            ...
            result.set(await expectFail(FANT.methods.pause().send({from: account7})).catch(catcher));
            result.checkDidFail();
            ...
            result.set(await expectFail(FANT.methods.pause().send({from: owner})).catch(catcher));
            result.checkDidFail();
            // Disallowed: non-pauser role unpausing the paused contract.
            result.set(await expectFail(FANT.methods.unpause().send({from: account7})).catch(catcher));
            result.checkDidFail();
            ...
            result.set(await expectFail(FANT.methods.unpause().send({from: owner})).catch(catcher));
            result.checkDidFail();
        });
    ...
    if (runThisTest())
        it('34. Test kill()', async () => {
            ...
            await subtest('34a. kill when not paused', async () => {
                result.set(await expectFail(FANT.methods.kill().send({from: account1})).catch(catcher));
                result.checkDidFail('Non-owner should not be able to kill');

                result.set(await expectFail(FANT.methods.kill().send({from: owner})).catch(catcher));
                result.checkDidFail('Owner should not be able to kill when not paused');
            }).catch(catcher);

            result.set(await FANT.methods.addPauser(account2).send({from: owner}));
            result.checkTransactionOk();
            result.set(await FANT.methods.removePauser(owner).send({from: owner}));
            result.checkTransactionOk();

            result.set(await FANT.methods.pause().send({from: account2}));
            result.checkTransactionOk();
            ...
            await subtest('34b. kill when paused, with correct owner param', async () => {
                result.set(await expectFail(FANT.methods.kill().send({from: account9})).catch(catcher));
                result.checkDidFail('Non-owner should not be able to kill');
                ...
                result.set(await FANT.methods.kill().send({from: pauser}).catch(catcher));
                result.checkTransactionOk;

                result.set(await expectFail(FANT.methods.symbol().call(),
    ...
    if (runThisTest())
        it('40. test uniform grantor setup', async () => {
            ...
            await subtest('40a. check that owner can\'t set restrictions on non-grantor.', async () => {
                result.set(await expectFail(FANT.methods.setRestrictions(grantor, MINSTARTDAY, MAXSTARTDAY, EXPIRATIONDAY).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40a1. check that grantor is not yet an exchange grantor.', async () => {
                result.set(await FANT.methods.isGrantor(grantor).call({from: account9}));
                result.checkIsEqual(false);
                result.set(await FANT.methods.isUniformGrantor(grantor).call({from: account9}));
                result.checkIsEqual(false);
            }).catch(catcher);

            await subtest('40b. check that owner can\'t set restrictions on unregistered grantor.', async () => {
                result.set(await FANT.methods.addGrantor(grantor, isUniformGrantor).send({from: owner}));
                result.checkTransactionOk();
                result.set(await expectFail(FANT.methods.setRestrictions(grantor, MINSTARTDAY, MAXSTARTDAY, EXPIRATIONDAY).send({from: owner}),
            ...
            await subtest('40c. check that owner can\'t set vesting schedule of unregistered grantor.', async () => {
                result.set(await expectFail(FANT.methods.setGrantorVestingSchedule(grantor, ...vestingSchedule).send({from: owner}),
                    'account not registered').catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40d. check that owner can\'t set vesting schedule of non-grantor.', async () => {
                result.set(await expectFail(FANT.methods.setGrantorVestingSchedule(beneficiary, ...vestingSchedule).send({from: owner}),
                    'account not registered').catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40e. check that grantor can\'t grant tokens without restrictions set up.', async () => {
                result.set(await expectFail(FANT.methods.grantUniformVestingTokens(beneficiary, ...grantParams).send({from: owner}),
            ...
            await subtest('40f. check that grantor can\'t self-set own restrictions.', async () => {
                result.set(await expectFail(FANT.methods.setRestrictions(grantor, MINSTARTDAY, MAXSTARTDAY, EXPIRATIONDAY).send({from: grantor}),
                    DEFAULT_ERROR).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40g. check that grantor can\'t self-set own vesting schedule.', async () => {
                result.set(await expectFail(FANT.methods.setGrantorVestingSchedule(grantor, ...vestingSchedule).send({from: grantor}),
                    DEFAULT_ERROR).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
            ...
            await subtest('40h. check that owner CAN set restrictions and vesting schedule on registered grantor.', async () => {
                ...
                result.set(await FANT.methods.registerAccount().send({from: grantor}));
                result.checkTransactionOk();
                result.set(await FANT.methods.setRestrictions(grantor, MINSTARTDAY, MAXSTARTDAY, EXPIRATIONDAY).send({from: owner}));
                result.checkTransactionOk();
                result.set(await FANT.methods.setGrantorVestingSchedule(grantor, ...vestingSchedule).send({from: owner}));
                result.checkTransactionOk();
            }).catch(catcher);
            ...
            await subtest('40i. check that vesting schedule cannot be changed', async () => {
                ...
                result.set(await expectFail(FANT.methods.setGrantorVestingSchedule(grantor, ...vestingSchedule2).send({from: owner}),
                    'schedule already exists').catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            // Fund grantor so it can make grants
            await FANT.methods.safeTransfer(grantor, tokensToWei(100 * ONE_MILLION)).send({from: owner});
            ...
            await subtest('40j. check that date restrictions are enforced when granting.', async () => {
                result.set(await expectFail(FANT.methods.grantUniformVestingTokens(beneficiary, ...grantParams_restricted1).send({from: grantor, gas: UNIFORMGRANTGAS}),
                    'startDay too early').catch(catcher));
                result.checkDidFail();
                result.set(await expectFail(FANT.methods.grantUniformVestingTokens(beneficiary, ...grantParams_restricted2).send({from: grantor, gas: UNIFORMGRANTGAS}),
                    'startDay too early').catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40k. check that grantor CAN grant tokens, and they are granted correctly.', async () => {
                result.set(await FANT.methods.grantUniformVestingTokens(beneficiary, ...grantParams).send({from: grantor, gas: UNIFORMGRANTGAS}));
                result.checkTransactionOk();
            ...
            await subtest('40m. check that under the same conditions, safe grant is disallowed.', async () => {
                result.set(await expectFail(FANT.methods.safeGrantUniformVestingTokens(beneficiary2, ...grantParams).send({from: grantor, gas: UNIFORMGRANTGAS}),
                    'account not registered').catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('40n. check that grantor CAN safe-grant tokens, and they are granted correctly.', async () => {
                result.set(await FANT.methods.registerAccount().send({from: beneficiary2}));
                result.checkTransactionOk();
                result.set(await FANT.methods.safeGrantUniformVestingTokens(beneficiary2, ...grantParams).send({from: grantor, gas: UNIFORMGRANTGAS}));
                result.checkTransactionOk();
            ...
            await subtest('40p. check revocation of beneficiary2.', async () => {
                result.set(await FANT.methods.revokeGrant(beneficiary2, MINSTARTDAY + 5).send({from: grantor, gas: REVOKEONGAS}));
                result.checkTransactionOk();
            ...
            await subtest('40r. heck that grantor account reflects one full and one partial revocation having been subtracted.', async () => {
                result.set(await FANT.methods.balanceOf(grantor).call());
    ...
    if (runThisTest())
        it('41. whoAmI()', async () => {
            ...
            await subtest('41a. be somebody.', async () => {
                result.set(await FANT.methods.iAm("The boss man").send({from: owner}));
                result.checkTransactionOk();

                result.set(await FANT.methods.iAm("just account 5").send({from: account5}));
                result.checkTransactionOk();

                result.set(await FANT.methods.iAm("I'm last! (and 31 bytes long..)").send({from: account9}));
                result.checkTransactionOk();
            }).catch(catcher);

            await subtest('41b. I am who I think I am.', async () => {
                result.set(await FANT.methods.whereAmI().call({from: owner}));
                result.checkIsEqual(owner);
                result.set(await FANT.methods.whoAmI().call({from: owner}));
                result.checkIsEqual("The boss man");

                result.set(await FANT.methods.whereAmI().call({from: account5}));
                result.checkIsEqual(account5);
                result.set(await FANT.methods.whoAmI().call({from: account5}));
                result.checkIsEqual("just account 5");

                result.set(await FANT.methods.whereAmI().call({from: account9}));
                result.checkIsEqual(account9);
                result.set(await FANT.methods.whoAmI().call({from: account9}));
    ...
    if (runThisTest())
        it('42. isRegistered()', async () => {
            ...
            result.set(await FANT.methods.isRegistered(owner).call({from: account9}));
            result.checkIsEqual(true);

            // Account1 should initially not be registered.
            result.set(await FANT.methods.isRegistered(account1).call({from: account9}));
            result.checkIsEqual(false);
    ...
    if (runThisTest())
        it('43. burn tokens - test not-vested tokens can\'t be burned', async () => {
            ...
            await subtest('43a. check that cannot burn not-vested tokens (using account1)', async () => {
                ...
                result.set(await expectFail(FANT.methods.burn(tokensToWei(1)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
            ...
            await subtest('43b. check that only vested tokens can be burned (using account2)', async () => {
                ...
                result.set(await expectFail(FANT.methods.burn(tokensToWei(500001)).send({from: account2})).catch(catcher));
                result.checkDidFail();

                // Try to burn exactly half the tokens (half are vested). Should work.
                result.set(await FANT.methods.burn(tokensToWei(500000)).send({from: account2}));
                result.checkTransactionOk();
            }).catch(catcher);
            ...
            await subtest('43b. check that fully vested tokens can be burned (using account3)', async () => {
                ...
                result.set(await expectFail(FANT.methods.burn(tokensToWei(1000001)).send({from: account3})).catch(catcher));
                result.checkDidFail();

                // Try to burn exactly all the tokens (all are vested). Should work.
                result.set(await FANT.methods.burn(tokensToWei(1000000)).send({from: account3}));
                result.checkTransactionOk();
            }).catch(catcher);
        });
